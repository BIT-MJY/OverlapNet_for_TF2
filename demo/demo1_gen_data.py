...
def show_images(depth_data, normal_data, intensity_data, semantic_data):
  ...
  # color the semantic class to better visualize   # 从20个类别中argmax绘图，其实每个像素位置均有20个值
...
if __name__ == '__main__':
  ...
  scan_folder = config['Demo1']["scan_folder"]   # 激光数据
  semantic_folder = config['Demo1']["semantic_folder"]   # 激光雷达对应的语义标签？
  dst_folder = config['Demo1']["dst_folder"]   # 4 kinds image文件，文件夹中包含.npy文件，是保存的各路图像。因为使用np.save保存，因此是npy文件
