...
from NormalizedCorrelation2D import NormalizedCorrelation2D  # 这个文件待修改
...
def generate360OutputkLegs(left_input, right_input, input_shape=(50, 50, 1), config={},
  ...
  if (smallNet):
    ...
    finalconv = Conv2D( filters=2, kernel_size=(5, 15), strides=5, padding='valid', activation='relu', \
                kernel_initializer='he_normal',name='s_conv1', kernel_regularizer=l2(2e-4), input_shape=input_shape) 
    l = finalconv(left_input)
    r = finalconv(right_input)
    return (l, r)
  else:
    ...
    # 首先我假定一个error map,形状即为input_shape
    # error_input = K.zeros(input_shape)
    print("virtual error propagation start")
    
    # 待修改回去================================别忘啦
    kernel_regularizer = None
    # l = Conv2D(16, (5, 15), strides=config['strides_layer1'], activation='relu',
    #                kernel_regularizer=kernel_regularizer, name="s_conv1l")(left_input)
    # r = Conv2D(16, (5, 15), strides=config['strides_layer1'], activation='relu',
    #                kernel_regularizer=kernel_regularizer, name="s_conv1r")(right_input)
    # print("第一层卷积")
    ...
    # conv1_error_prop = Conv2D(16, (5, 15), strides=config['strides_layer1'], activation='relu', input_shape=input_shape,
    #                kernel_regularizer=kernel_regularizer, name="s_conv1_error", trainable=False)
...
def generateCorrelationHead(encoded_l, encoded_r, config={})  :
    ...
    """
    norm_corr = NormalizedCorrelation2D(output_dim=1, normalize='none')([encoded_l, encoded_r])   # norm_corr为(1,360,1)
    ...
    # print("norm_corr维度：", K.int_shape(norm_corr))   # （1,Dimension(360)）
...
def generateSiameseNetworkTemplate(input_shape=(50, 50, 1), config={}, smallNet=False):
  ...
  # Define the input
  print("准备建立模型")
  left_input = Input(input_shape,name='inputl')
  right_input = Input(input_shape,name='inputr')
  ...
  # 建立误差模型
  # siamese_net = Model(inputs=[left_input_error, right_input_error], outputs=[prediction_overlap_error, prediction_orientation_error])
