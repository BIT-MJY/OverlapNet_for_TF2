...
class Infer():
  ...
  def __init__(self, config):
    ...
    # Input for leg
    leg_input_l = Input(self.inputShape)
    leg_input_r = Input(self.inputShape)
    ...
    self.leg = Model(inputs=leg_input_l, outputs=encoded_l)
    
    # Input for head
    inputHead_l = Input(encoded_l.shape[1:])
    inputHead_r = Input(encoded_r.shape[1:])
    ...
    self.head = Model(inputs=(inputHead_l, inputHead_r), outputs=[overlap_predict, orientation_predict])
    ...
    if len(pretrained_weightsfilename) > 0:
      ...
      self.head.load_weights(pretrained_weightsfilename, by_name=True)   # 怎么仿佛能够自动识别并加载对应的权重，其实是这样的，详情见https://blog.csdn.net/qq_24221615/article/details/104567116
    else:
  ...
  def infer_multiple_vs_multiple(self, file_names, first_idxs, second_idxs):
    ...
    if len(second_idxs) > 0:
      ...
      test_generator_head = ImagePairOverlapSequenceFeatureVolume(pair_indizes, np.zeros((len(pair_indizes))),
                                                                  self.batch_size, np.array(self.feature_volumes))
      model_outputs = self.head.predict_generator(test_generator_head, max_queue_size=10,
      # model_outputs = self.head.predict(test_generator_head, max_queue_size=10,
                                                  workers=8, verbose=1)
